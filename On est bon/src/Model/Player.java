...
public class Player extends Character {
	...
	public void getBonus(Potion potion) {
		if (potion.getBonusHealth() != 0) {
			healPotions.add(potion);
		} else if (potion.getBonusMana() != 0) {
			manaPotions.add(potion);
		} else if (potion.isBonusInvincible() == true) {
			becomeInvincible(500);
		}
	}

	public void useHealPotion() {
		if (healPotions.size() != 0) {
			this.setHealth(this.getHealth() + healPotions.get(0).getBonusHealth());
			healPotions.remove(0);
		}
	}

	public void useManaPotion() {
		if (manaPotions.size() != 0) {
			this.setMana(this.getMana() + manaPotions.get(0).getBonusMana());
