...
public class Player extends Character {
	...
	public void attack() {
		if (this.getMana() >= 5) {
			game.addProjectile(this.getPosX(), this.getPosY(), dir, speed * 3, this.attackStat);
			useMana(5);
		}
	}
	...
	public void getBonus(Potion potion) {
		...
		if (potion.isBonusInvincible() == true) {
			becomeInvincible(2000);
		}
	}

	void becomeInvincible(int delay) {
		this.setInvincible(true);
		this.setAttackStat(2*attackStat);
		this.tick = 0;
	}
	
	void becomeNormal(){
		setInvincible(false);
		this.setAttackStat(attackStat/2);
	}
