...
public class Player extends Character {
    ...
    private boolean swordAttacking;
	...
	private void getBonus(Potion potion) {
		if (inventory.notFull() && ! potion.isBonusInvincible()) {
			inventory.addPotion(potion);
		} else if (potion.getType().equals("health")) {
			this.setHealth(this.getHealth() + potion.getBonusHealth());
		} else if (potion.getType().equals("mana")) {
			this.setMana(this.getMana() + potion.getBonusHealth());
		} else if (potion.getType().equals("invincible")) {
	...
	private void becomeInvincible(int tickNumber) {
	...
	public void run() {
		while (!dead) {
			try {
				...
				if (tick1 >= limitTick && this.isInvincible()) {
					this.becomeNormal();
				}
				if (this.isChangingMap()) {
					tick2 += 1;
				}
				if (tick2 >= TICK_CHANGE_MAP && this.isChangingMap()) {
