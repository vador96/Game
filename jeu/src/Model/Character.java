...
public abstract class Character implements Collidable, Collision, Subject, Runnable {
	...
	private void generateHitbox() {
		// hitbox is the main hitbox of the character
        // rtop is a hitbox to detect collision from the up side,
        // it is the same logic for rbot, rleft and rright
        // fieldOfAction is the rectangle that is in front of the player
	...
	public void setHealth(int health) {
        // health can't be negative and can't be bigger than maxHealth
		if (health >= maxHealth) {
	...
	public void setMana(int mana) {
        // mana can't be negative and can't be bigger than maxMana
		if (mana >= maxMana) {
			this.mana = maxMana;
		} else if (mana <= 0) {
			this.mana = 0;
	...
	public void setFieldOfAction() {
        // update the position of the rectangle in front of the character according to his direction
        if(dir == 0) {
    ...
    public boolean targetSpotted(Character target) {
        // target is spotted if he is the fieldOfAction of the character
        boolean spotted;
	...
	private int collidesOnSide(Collidable collidable) {
        // return the side which is collided when a collision is performed
	...
	public void goBack(Collidable collidable) {
        // eject the character on the opposite direction
	...
	public void update() {
        // update all the hitbox and notify the game
