...
public class Game implements Observer, Subject {
	...
	private synchronized void loadLevel(String nameLevel) {
		//remove all the monsters of the level and begin a new level
        // with its matrix and its collidables
	...
	public void changeLevelTo(char direction) {
        // choose the logical next level
        // using the world's matrix of the World class
	...
	private void generateCollidables() {
        // generate all the collidables in the position chosen
        // in the file. Each class correspond to a char type
	...
	private void checkCollision() {
        // check all the collisions between classes
        // which implements Collision and Collidable interfaces
	...
	private void sendToGraveyard() {
        // remove the monsters that have their dead-attribute on true
        // remove the potions that have their consummed-attribute on true
		for (int j = 0; j < collidables.size(); j++) {
			for (int i = 0; i < monsters.size(); i++) {
